...
* `CdcSinkLinkedTableConfig` only creates the document ID reference in the current
  document. It does **not** create or maintain the referenced document itself.
  The referenced document exists only if the referenced table is also configured
  as a root table.
...
and the join column values. The `JoinColumns` values are appended to
`LinkedCollectionName` in the order listed. Use the same order as the referenced
table's `PrimaryKeyColumns`, so the generated ID matches the referenced document ID.

This configuration only writes the `"Customers/42"` reference into the Orders document.
The `Customers/42` document itself exists only if the `customers` table is also
configured as a root table. Load the referenced document using
...
| Updates to referenced data | Reflected via CDC | Reflected via CDC only when the referenced table is also configured as a root table |
