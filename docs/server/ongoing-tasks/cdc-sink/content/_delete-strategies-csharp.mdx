...
* In this article:
  ...
  * [Pattern: audit trail](#pattern-audit-trail-root-document)
...
<Panel heading="Default behavior">
...
(that is, `OnDelete.IgnoreDeletes = true` with no `OnDelete.Patch`). If a delete patch is
configured, the event is still routed so the patch can run - even when `IgnoreDeletes = true`.
...
<Panel heading="Pattern: audit trail (root document)">
...
<Panel heading="Pattern: silent ignore">
...
* `IgnoreDeletes = true` prevents the item from being removed from the array/map/value
...
* [REPLICA IDENTITY](../../../../server/ongoing-tasks/cdc-sink/source-database-setup/postgres/replica-identity.mdx) (PostgreSQL)
