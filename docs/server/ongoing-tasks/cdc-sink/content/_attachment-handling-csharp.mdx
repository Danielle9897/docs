...
* Binary columns (`bytea`, `varbinary`, etc.) and text columns (`text`, `nvarchar`, etc.)
...
runtime value type (not the SQL column type): `application/octet-stream` for binary
values, `text/plain` for text values.
