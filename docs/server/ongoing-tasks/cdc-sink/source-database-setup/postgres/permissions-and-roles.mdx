...
The **CDC Sink user** is the PostgreSQL role in the connection string that the CDC Sink
task uses to connect to the source database. Throughout this page, `cdc_user` is used as
the example username for this role.
...
Creating and dropping logical replication slots requires the `REPLICATION` role attribute:
...
On PostgreSQL 10 - 13, replication slot management effectively requires `SUPERUSER`.
...
-- If you want CDC Sink to manage replication slots automatically:
-- (the REPLICATION attribute is already granted above)
