...
* To verify in another session that the document still has this change vector,  
  pass the document ID and change vector to `RegisterForConcurrencyCheck`.  
  Learn more in [Register a concurrency check without loading the document](../../../../client-api/session/configuration/how-to-enable-optimistic-concurrency.mdx#register-a-concurrency-check-without-loading-the-document).
