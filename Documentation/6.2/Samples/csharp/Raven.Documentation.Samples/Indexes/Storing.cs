...
namespace Raven.Documentation.Samples.Indexes
{
    public class StoringDataInIndex
    {
            ...
            AbstractIndexCreationTask<Order, QuantityOrdered_ByCompany.IndexEntry>
        {
            ...
            // Project the following details when querying the index:
            // (Note: it is not mandatory for this class to be within the index class)
        ...
        {
            // from index from the stored field, but document still loads from the store... becuase we needed to answer the above 2 fields
            public int TotalItemsOrdered { get; set; }
        }
        #endregion
        ...
        {
            // from document because it was not stored in the index enen though it is an index-field
            public string Company { get; set; }
            // from document - not an inex field
            public DateTime OrderedAt { get; set; }
            // from index from the stored field, but document still loads from the store... becuase we needed to answer the above 2 fields
            public int TotalItemsOrdered { get; set; }
        }
